$(document).ready(async function () {
    ...
    axios.get('../auth/api/?action=etusivu&lang=en')
        .then(async function (response) {
            ...
            <div class="cursor-pointer text-sm mt-4 font-semibold tracking-tight text-gray-900" data-modal-target="${element?.id}-modal" data-modal-toggle="${element?.id}-modal" type="button">Pricing</div>
        </div>
            ...
            <div class="relative bg-white rounded-lg shadow">
                ...
                <div class="p-6 space-y-6">
                    <p class="text-base leading-relaxed text-gray-500">
                        ${element?.lisatieto_en ?? element?.lisatieto}
                    </p>
                </div>
            </div>
        </div>
