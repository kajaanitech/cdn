        ...
        .then(async function (response) {
            ...
            response.data.tuotteet.forEach(element => {
                ...
                if (Array.isArray(element?.kuva)) {
                    ...
                    <div data-popover id="${element?.id}-description" role="tooltip" class="absolute z-10 invisible inline-block text-sm font-light text-gray-500 transition-opacity duration-300 bg-white border border-gray-200 rounded-lg shadow-sm opacity-0 w-72">
                        <div class="p-3 space-y-2">
                            <h3 class="font-semibold text-gray-900 dark:text-white">${element?.nimi} - Hinnoittelu</h3>
                            <p>${element?.lisatieto}</p>
                        </div>
